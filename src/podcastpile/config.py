...
class Config:
    ...
    WORKER_AUTH_ENABLED: bool = os.getenv("WORKER_AUTH_ENABLED", "false").lower() == "true"
    WORKER_PASSWORD: Optional[str] = os.getenv("WORKER_PASSWORD")

    # Admin Authentication
    ADMIN_AUTH_ENABLED: bool = os.getenv("ADMIN_AUTH_ENABLED", "false").lower() == "true"
    ...
    def validate(cls):
        ...
        if cls.WORKER_AUTH_ENABLED and not cls.WORKER_PASSWORD:
            raise ValueError("WORKER_PASSWORD must be set when WORKER_AUTH_ENABLED is true")
        if cls.ADMIN_AUTH_ENABLED and not cls.ADMIN_PASSWORD:
            raise ValueError("ADMIN_PASSWORD must be set when ADMIN_AUTH_ENABLED is true")
